...
class EncoderLayer(nn.Module):
    ...
    # The process involves handling the input sequence through a series of operations to enhance the features within the
    # sequence, providing a richer feature representation for the decoder layer.
...
class DecoderLayer(nn.Module):
    ...
    #  e_output: encoder output, (B, L, d_model).
    def forward(self, x, e_output, e_mask, d_mask):
        ...
        return x  # (B, L, d_model)  # 经过解码层处理后的序列特征表示
...
class MultiheadAttention(nn.Module):
    ...
    #  q, k, v: (B, L, d_model)  Implemented multi-head attention
    def forward(self, q, k, v, mask=None):
        ...
        return self.w_0(concat_output)  # (B, L, d_model)  # 输入序列中每个元素在考虑了其他所有元素的上下文信息后的综合特征表示
    ...
    def self_attention(self, q, k, v, mask=None):
        ...
        return attn_values  # (B, num_heads, L, d_k)  d_k=d_model // num_heads, the dimension of the keys (and queries)
        # in the self-attention mechanism.多头注意力机制得到的注意力分数
